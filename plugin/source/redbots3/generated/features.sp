...
enum
{
	...
	FEATURE_MEDIC_HEALS_IN_BREAK,
	FEATURE_MEDIC_RANKS_HURT,
	FEATURE_MEDIC_RANKS_RANGE,
	FEATURE_MEDIC_RANKS_CALLERS_ONLY,
	FEATURE_MEDIC_UBERS_EARLY,
	FEATURE_COUNT
}
...
static const char FEATURE_NAME[FEATURE_COUNT][] =
{
	...
	"medic_heals_in_break",
	"medic_ranks_hurt",
	"medic_ranks_range",
	"medic_ranks_callers_only",
	"medic_ubers_early"
};
...
void LoadFeatures()
{
	...
	/* On, measured 2026-09-12 on Decoy advanced, three attempts each of two waves. Every
	...
	g_arrFeatureConVars[FEATURE_MEDIC_HEALS_IN_BREAK] = MakeFeature(FEATURE_MEDIC_HEALS_IN_BREAK,
		"The medic heals through the break rather than standing at the front, so he brings a charge into the wave.");

	/* Off, and it has nothing to decide while a Heavy is alive. MEDIC_PATIENT_HEAVY_WORTH is
	...
	g_arrFeatureConVars[FEATURE_MEDIC_RANKS_HURT] = MakeFeature(FEATURE_MEDIC_RANKS_HURT,
		"A teammate missing health is worth more medigun than the same man whole.", false);

	/* Off, and like medic_ranks_hurt it has nothing to decide while a Heavy is alive, for the
	...
	g_arrFeatureConVars[FEATURE_MEDIC_RANKS_RANGE] = MakeFeature(FEATURE_MEDIC_RANKS_RANGE,
		"The walk to a teammate is charged against what the medigun is worth on him.", false);

	/* Off, and it is the only one of the three terms that can change who the ranking picks while
	...
	g_arrFeatureConVars[FEATURE_MEDIC_RANKS_CALLERS_ONLY] = MakeFeature(FEATURE_MEDIC_RANKS_CALLERS_ONLY,
		"Only a player who called outranks the rest. A player who has not asked takes his turn with everybody else.");

	/* Off, and three arms say the button was never the problem. The wave total it was written
	...
	g_arrFeatureConVars[FEATURE_MEDIC_UBERS_EARLY] = MakeFeature(FEATURE_MEDIC_UBERS_EARLY,
		"The charge goes off when the patient is in a fight that can kill him, not when he is already at half health.", false);
