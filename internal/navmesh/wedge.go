...
func (v PointVerdict) Suspicious() bool {
	return v.DeadEnd || v.AllExitsFall || len(v.Stacked) > 1 ||
		(v.Under != nil && v.NarrowestSide < PlayerWidth)
}

// String is the verdict as one report line.
func (v PointVerdict) String() string {
	if v.Under == nil {
		return fmt.Sprintf("%.0f %.0f %.0f: off the mesh, nearest surface %.0f away",
			v.Pos.X, v.Pos.Y, v.Pos.Z, v.NearestDistance)
	}
