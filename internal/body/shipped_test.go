...
var reshaped = map[string]string{
	...
	"Event_PlayerTeam":                 "two calls more than the shipped one, deliberately: a managed fake client that moves off RED now has its buildings cleared and is kicked so the occupied client slot is freed for the existing fill timer. The direct test covers that transition and proves disconnects, ordinary robots, healthy defenders and humans are unchanged",
	...
	"OnPluginStart":                    "the branches of the shipped one that compile. Five of its preprocessor blocks are guarded by TESTING_ONLY and IDLEBOT_AIMING, commented out at tf2_defenderbots.sp lines 23 and 39 and written nowhere else, so the testing preference path and its banner, the sm_bots_start_now command, the upgrade-address LogMessage and InitTFBotAim have never run. MOD_REQUEST_CREDITS, MOD_ROLL_THE_DICE_REVAMPED and METHOD_MVM_UPGRADES are defined at lines 25, 27 and 29, so all three of those are ported. Every convar creation, registration, hook and load is here, in order, and nothing else is",
	"InitOffsets":                      "the ten SetOffset calls and not the dump after them. The dump is #if defined TESTING_ONLY, commented out at tf2_defenderbots.sp line 23 and written nowhere else, so its ten LogMessage lines have never run. The ten SetOffset calls are unchanged, same classes, same props, same order",
	"InitMvMUpgrades":                  "four LogMessage lines short of the shipped one, under #if defined TESTING_ONLY, commented out at tf2_defenderbots.sp line 23 and written nowhere else, so they have never printed. The three offsets and the size computed from them are unchanged",
	"DrawWardrobe":                     "one LogMessage more than the shipped one, and it is a fix rather than a port: mvm-6gi. Both reasons for dropping a hat end in DropHatFromPool, which logs the same line for each, so nothing could tell a hat with no model from one the game refused to attach. Only the second throws. Everything else is unchanged, and the shipped body is the one the port carried across before this line was added", "InitSDKCalls": "one preparation short of the shipped one: CTFWeaponBaseGun::GetProjectileGravity sits under #if defined IDLEBOT_AIMING, commented out at tf2_defenderbots.sp line 39 and written nowhere else, so it has never been prepared and GetProjectileGravity has never been called. The other fifteen are unchanged, same call kind, same source, same parameters in the same order, same message on failure",
	"DHookCallback_LoadUpgradesFile_Post": "one LogMessage short of the shipped one, under #if defined TESTING_ONLY, commented out at tf2_defenderbots.sp line 23 and written nowhere else, so it has never printed. The address it catches and the guard around it are unchanged", "Timer_PlayerSpawn": "one call short of the shipped one: VS_AddBotAttribute(data, CTFBot_IGNORE_ENEMIES) sits under #if defined IDLEBOT_AIMING, which is defined nowhere in the include tree, so it never ran. The snapshot is the text, guards and all; what compiled is what the port carries",
