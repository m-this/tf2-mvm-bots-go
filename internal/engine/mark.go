...
func (v Vision) KnownCount(team Team) int32 { return marks.KnownCount(v, team) }

// GetKnown is what the bot remembers about that entity, and nothing when it has
// not noticed it.
//
//sp:method GetKnown
func (v Vision) GetKnown(entity int32) Known { return marks.GetKnown(v, entity) }

// ForgetAllKnownEntities empties what the bot has noticed.
//
//sp:method ForgetAllKnownEntities
func (v Vision) ForgetAllKnownEntities() { marks.ForgetAllKnownEntities(v) }

// AddKnownEntity puts one back, which is how the bot is made to look at it.
//
//sp:method AddKnownEntity
func (v Vision) AddKnownEntity(entity int32) { marks.AddKnownEntity(v, entity) }

// RandomInt is the game's own randomness, which mvm-z83.18 has yet to make a
// parameter.
//
//sp:native GetRandomInt
func RandomInt(low int32, high int32) int32 { return marks.RandomInt(low, high) }

// EquipWeaponSlot puts whatever is in that slot in the bot's hands.
//
//sp:body EquipWeaponSlot
func EquipWeaponSlot(client int32, slot int32) bool { return marks.EquipWeaponSlot(client, slot) }
...
func IsClientConnected(client int32) bool { return marks.IsClientConnected(client) }
