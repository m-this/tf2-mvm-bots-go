...
//
//sp:method JumpToKey
func (kv KeyValues) JumpToKey(key string, create bool) bool { return prefs.JumpToKey(kv, key, create) }
...
//
//sp:method Rewind
func (kv KeyValues) Rewind() { prefs.Rewind(kv) }

// Num is a number under the current key, or the fallback.
//
//sp:method GetNum
func (kv KeyValues) Num(key string, fallback int32) int32 { return prefs.KVNum(kv, key, fallback) }
...
//
//sp:method SetNum
func (kv KeyValues) SetNum(key string, value int32) { prefs.SetNum(kv, key, value) }

// SetNumText writes one under a key that was handed in.
//
//sp:method SetNum
func (kv KeyValues) SetNumText(key Text, value int32) { prefs.SetNumText(kv, key, value) }

// String is text under the current key.
//
//sp:method GetString sized
func (kv KeyValues) String(key string) (out Text) { return prefs.KVString(kv, key) }

// SectionName is the name of the key it is standing on.
//
//sp:method GetSectionName sized
func (kv KeyValues) SectionName() (out Text) { return prefs.SectionName(kv) }

// GotoFirstSubKey steps into the first child.
//
//sp:method GotoFirstSubKey
func (kv KeyValues) GotoFirstSubKey() bool { return prefs.GotoFirstSubKey(kv) }

// GotoNextKey steps to the next sibling.
//
//sp:method GotoNextKey
func (kv KeyValues) GotoNextKey() bool { return prefs.GotoNextKey(kv) }
...
func StringToInt(text Text) int32 { return prefs.StringToInt(text) }

// IntToString writes one into a buffer.
//
//sp:native IntToString sized
func IntToString(value int32) (written int32, out Text) { return prefs.IntToString(value) }

// PushString adds text to a list of text.
//
//sp:method PushString
func (l List) PushString(text string) { prefs.PushString(l, text) }

// NewStringList makes a list whose cells hold text rather than a number.
//
//sp:new ArrayList
func NewStringList(blockSize int32) List { return prefs.NewStringList(blockSize) }

// ClientTeam is the team as TFTeam rather than as a number.
//
//sp:native TF2_GetClientTeam
func ClientTeam(client int32) Team { return prefs.ClientTeam(client) }
...
func AddRandomDefenderBots(amount int32) { prefs.AddRandomDefenderBots(amount) }
...
func IsServerFull() bool { return prefs.IsServerFull() }
